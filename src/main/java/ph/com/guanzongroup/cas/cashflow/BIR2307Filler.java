...
public class BIR2307Filler {
     ...
     */
    public void fillForm() throws IOException, InvalidFormatException, CloneNotSupportedException, SQLException, GuanzonException {
        if (!inputFile.exists()) {
    ...
    private JSONObject detailSection() throws SQLException, GuanzonException, CloneNotSupportedException {
        ...
        int detailCount;
        try {
            detailCount = poDisbursementController.getDetailCount();
        } catch (NullPointerException e) {
            poJSON.put("result", "error");
            poJSON.put("message", "Transaction details not available.");
            return poJSON;
        }
...
//        int detailCount = poDisbursementController.getDetailCount();
    System.out.println("Detail count = " + detailCount);
            ...
            for (int lnctr = 0; lnctr < detailCount; lnctr++) {
                ...
                poJSON = getDisbursementSourceDate(SourceNo, SourceCode);
                if ("error".equals(poJSON.get("result"))) {
                    return (JSONObject) poJSON.get("message");
                }
